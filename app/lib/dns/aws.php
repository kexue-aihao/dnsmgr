...
class aws implements DnsInterface
{
    ...
    public function getDomainRecords($PageNumber = 1, $PageSize = 20, $KeyWord = null, $SubDomain = null, $Value = null, $Type = null, $Line = null, $Status = null)
    {
        ...
        if (!isNullOrEmpty($Type)) {
            $type = $this->convertType(strtoupper($Type));
            $list = array_values(array_filter($list, function ($v) use ($type) {
                return $v['Type'] === $type;
            }));
        }
        if (!isNullOrEmpty($Status) && (string) $Status !== '1') {
            $list = [];
        }
    ...
    public function getDomainRecordInfo($RecordId)
    {
        ...
        $set = $this->getRecordSet($this->toFqdn($old['Name']), $old['Type']);
        if ($set === false) {
            return false;
        }
        if (!$set || !in_array($old['Value'], $this->extractValues($set), true)) {
            $this->setError('记录已变更或不存在，请刷新记录后重试');
            return false;
        }
    ...
    public function updateDomainRecord($RecordId, $Name, $Type, $Value, $Line = 'default', $TTL = 600, $MX = 1, $Weight = null, $Remark = null)
    {
        ...
        $oldType = $old['Type'];
        $oldRaw = $old['Value'];
        $oldSet = $this->getRecordSet($oldFqdn, $oldType);
        if ($oldSet === false) {
            return false;
        }
        $oldValues = $oldSet ? $this->extractValues($oldSet) : [];
        if (!in_array($oldRaw, $oldValues, true)) {
            $this->setError('记录已变更或不存在，请刷新记录后重试');
            return false;
        }
        ...
        if (strcasecmp($oldFqdn, $fqdn) === 0 && strtoupper($oldType) === strtoupper($type)) {
            ...
            return $updated ? $this->encodeRecordId($this->fromFqdn($fqdn), $type, $newRaw) : false;
        }

        $targetSet = $this->getRecordSet($fqdn, $type);
        if ($targetSet === false) {
            return false;
        }
        $targetValues = $targetSet ? $this->extractValues($targetSet) : [];
        if (in_array($newRaw, $targetValues, true)) {
            $this->setError('已存在相同记录');
            return false;
        }
        $remaining = array_values(array_filter($oldValues, function ($item) use ($oldRaw) {
            return $item !== $oldRaw;
        }));
        $targetValues[] = $newRaw;
        // Route 53 同一 ChangeBatch 原子提交，改名或改类型失败时保留原记录。
        $updated = $this->changeRecordSets([
            [
                'Action' => $remaining ? 'UPSERT' : 'DELETE',
                'Name' => $oldFqdn,
                'Type' => $oldType,
                'TTL' => intval($oldSet['TTL']),
                'Values' => $remaining ?: $oldValues,
            ],
            [
                'Action' => 'UPSERT',
                'Name' => $fqdn,
    ...
    private function getRecordSet($fqdn, $type)
    {
        ...
        if (isset($set['AliasTarget']) || !empty($set['SetIdentifier'])) {
            $this->setError('暂不支持修改 Alias 或高级路由策略记录');
            return false;
        }
        return $set;
    }
